...
impl <'a> BitIterator<'a> {
    pub fn new(data: &'a [u8]) -> Self {
        BitIterator {data, byteidx: 0, bitidx: 7}
    }
}
...
impl <'a> Iterator for BitIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.byteidx >= self.data.len() { return None };
...
impl<I,T> BatchIterator<I> where I: Iterator<Item=T>,
{
...
impl<I,T> Iterator for BatchIterator<I> where I: Iterator<Item=T>,
{
    type Item = Vec<Option<T>>;

    fn next(&mut self) -> Option<Self::Item> {
        let mut batch = self.iter.by_ref().take(self.size).map(Some).collect::<Vec<_>>();
        if       batch.len() == 0 {None}
        else if  batch.len() == self.size {Some(batch)}
        else {
